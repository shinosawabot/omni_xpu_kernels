...
def _rms_kitchen_rope1(
    ...
    *,
    ...
    if scale.dtype != x.dtype:
        # Torch promotes mixed-dtype RMSNorm before RoPE. The fused kernel
        # rounds the normalized value to x.dtype, which changes this contract.
        normalized = torch.nn.functional.rms_norm(x, (x.shape[-1],), scale, epsilon)
        width = rot_dim or x.shape[-1]
        rotate = apply_kitchen_rope_split_half1 if split_half else apply_kitchen_rope1
        result = rotate(normalized[..., :width], freqs_cis)
        if width != x.shape[-1]:
            result = torch.cat((result, normalized[..., width:]), dim=-1)
        if inplace:
            x.copy_(result)
            return x
        return result
...
def _rms_kitchen_rope(
    ...
    *,
    ...
    if k_scale is None:
        k_scale = q_scale
    if q_scale.dtype != q.dtype or k_scale.dtype != k.dtype:
        return (
            _rms_kitchen_rope1(q, freqs_cis, q_scale, epsilon, split_half=split_half, rot_dim=rot_dim, inplace=inplace),
            _rms_kitchen_rope1(k, freqs_cis, k_scale, epsilon, split_half=split_half, rot_dim=rot_dim, inplace=inplace),
        )
    if torch.compiler.is_compiling():
        if inplace:
