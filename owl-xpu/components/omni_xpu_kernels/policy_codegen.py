...
EXPECTED_BUILD_TARGETS = ("bmg", "ptl-h", "lnl")
...
def render_tuning_header(manifest):
    ...
    for target in EXPECTED_BUILD_TARGETS:
        if target != "bmg":
            macro = {"ptl-h": "OMNI_XPU_ARCH_PTL_H", "lnl": "OMNI_XPU_ARCH_LNL"}[target]
            lines.extend([f"#elif defined({macro})"])
