...
CUTLASS-SYCL attention backend behind a small PyTorch API. Wheels are compiled
for one Torch minor and one GPU architecture; they are not portable across
those native ABI boundaries. The Windows `dev` line includes the separately
validated Lunar Lake (`lnl`) AOT target.
...
omni_xpu_kernel-0.2.0b2+torch213.lnl
```
