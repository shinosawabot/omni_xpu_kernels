// Parameterized BF16-IO Flash Attention kernel — bf16 input/output, fp32 S*V accumulation
// Based on flash.attn.b.mha.bf16io.opt.h with fp32 accumulator for S*V:
//   Q/K stay bf16 (no conversion), V converted bf16->fp16 before SLM store.
//   QK uses bf16 DPAS (same throughput as fp16), S*V uses fp16 inputs + fp32 accumulator.
...
ESIMD_INLINE void flashAttnBMhaBf16IoFp32AccumPrecomputed(
  uint8_t* qState,
  uint8_t* kState,
  uint8_t* vState,
  uint8_t* normAlpha,
  ...
  for (int kk = 0; kk < HD_HALF; kk++) {
    ...
    bf16QState.select<16, 2>(32 * kk) = f32Temp.select<16, 1>(0);
    bf16QState.select<16, 2>(32 * kk + 1) = f32Temp.select<16, 1>(16);
  }
